...
function ChoiceCard({ icon, title, desc, selected, onClick, small }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      background: selected ? 'rgba(45,184,122,0.08)' : '#FFFFFF',
      border: `1.5px solid ${selected ? '#2DB87A' : '#E8E6E3'}`,
      cursor: 'pointer', transition: 'all 0.25s', textAlign: 'left',
      boxShadow: selected ? '0 0 0 3px rgba(45,184,122,0.08)' : 'none',
      fontFamily: "'DM Sans', sans-serif",
    }}>
      <span style={{ fontSize: small ? 22 : 28, flexShrink: 0 }}>{icon}</span>
      <div style={{ flex: 1 }}>
        <p style={{ fontSize: small ? 14 : 16, fontWeight: 500, color: '#1A1A1A', marginBottom: desc ? 2 : 0 }}>{title}</p>
        {desc && <p style={{ fontSize: small ? 12 : 13, color: '#6B6B6B', lineHeight: 1.4 }}>{desc}</p>}
      </div>
      {selected && <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#2DB87A', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
export default function OnboardingPage() {
  ...
  const S = {
    ...
    hint: { fontSize: 12, color: '#9B9B9B', marginTop: 10, marginLeft: 4 },
    card: { background: '#FFFFFF', borderRadius: 18, border: '1px solid #E8E6E3', padding: 22, boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.03)' },
    chip: (sel: boolean) => ({
      padding: '8px 16px', borderRadius: 20,
      background: sel ? '#2DB87A' : '#FFFFFF',
      border: `1.5px solid ${sel ? '#2DB87A' : '#E8E6E3'}`,
    ...
    timeBtn: (sel: boolean) => ({
      ...
      background: sel ? '#2DB87A' : '#F5F5F3',
      border: `1px solid ${sel ? '#2DB87A' : '#E8E6E3'}`,
      color: sel ? 'white' : '#1A1A1A',
  ...
  return (
    <div style={{ background: '#FAFAF9', minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'DM Sans', sans-serif", color: '#1A1A1A' }}>
      {/* Progress bar */}
      {step > 0 && step <= TOTAL_STEPS && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 10 }}>
          <div style={{ height: 3, background: '#E8E6E3' }}>
            <div style={{ height: '100%', background: 'linear-gradient(90deg, #2DB87A, #34D399)', width: `${progress * 100}%`, transition: 'width 0.6s cubic-bezier(0.4,0,0.2,1)', borderRadius: '0 2px 2px 0' }} />
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: 480 }}>
          ...
          {step === 0 && <div style={{ textAlign: 'center', animation: 'fadeIn 0.8s ease' }}>
            ...
            <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 42, fontWeight: 400, letterSpacing: -0.5, marginBottom: 12, lineHeight: 1.1 }}>
              <TypeWriter text="Hello. I'm Pulse." speed={50} onDone={() => setTyped(true)} />
            </h1>
            {typed && <div style={{ animation: 'fadeIn 0.6s ease' }}>
              <p style={{ fontSize: 17, color: '#6B6B6B', lineHeight: 1.6, marginBottom: 40, maxWidth: 360, margin: '0 auto 40px' }}>
                Your personal assistant for life — tasks, goals, relationships, and everything in between.
              </p>
              <button onClick={next} style={{ padding: '16px 48px', borderRadius: 16, background: '#2DB87A', color: 'white', border: 'none', fontSize: 16, fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 16px rgba(45,184,122,0.3)', fontFamily: "'DM Sans', sans-serif" }}>
                Let's get started
              </button>
              <p style={{ fontSize: 12, color: '#9B9B9B', marginTop: 16 }}>Takes about 3 minutes</p>
            </div>}
          </div>}

          {/* STEP 1: Name */}
          {step === 1 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B' }}>First — what should I call you?</p></div>
            <input type="text" placeholder="Your first name" value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && canNext() && next()} autoFocus
              style={{ width: '100%', padding: '18px 22px', borderRadius: 16, border: `1.5px solid ${name ? '#2DB87A' : '#E8E6E3'}`, background: '#FFFFFF', fontSize: 20, fontWeight: 500, color: '#1A1A1A', fontFamily: "'DM Sans', sans-serif", boxShadow: name ? '0 0 0 3px rgba(45,184,122,0.08)' : 'none', transition: 'all 0.2s' }} />
            <p style={S.hint}>This is how I'll greet you each day</p>
          </div>}

          {/* STEP 2: Response Mode */}
          {step === 2 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B' }}>How should I talk to you?</p></div>
          ...
          {step === 3 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B', lineHeight: 1.5 }}>Who matters most to you? I'll only track the people you select.</p></div>
            <div style={{ ...S.card, marginBottom: 12 }}>
              <p style={{ fontSize: 11, fontWeight: 600, color: '#9B9B9B', marginBottom: 12, letterSpacing: 0.3, textTransform: 'uppercase' as const }}>From your contacts</p>
          ...
          {step === 4 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B' }}>{selectedContacts.length > 0 ? 'How often for each person?' : 'No contacts selected — you can add later.'}</p></div>
          ...
          {step === 5 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B', lineHeight: 1.5 }}>Can I check your messages? I only look at dates — never content.</p></div>
            <div style={S.card}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                <span style={{ fontSize: 24 }}>🔒</span>
                <div><p style={{ fontSize: 14, fontWeight: 600 }}>Privacy first</p><p style={{ fontSize: 12, color: '#6B6B6B' }}>Only tracks last-contact dates for your selected people.</p></div>
              </div>
          ...
          {step === 6 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B' }}>What are you working towards?</p></div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
              {GOAL_OPTIONS.map(g => <button key={g.id} onClick={() => toggle(goals, setGoals, g.id)} style={{
                ...
                background: goals.includes(g.id) ? 'rgba(45,184,122,0.08)' : '#FFFFFF',
                border: `1.5px solid ${goals.includes(g.id) ? '#2DB87A' : '#E8E6E3'}`,
                cursor: 'pointer', transition: 'all 0.2s', textAlign: 'left' as const, fontFamily: "'DM Sans', sans-serif",
              }}>
                <span style={{ fontSize: 20 }}>{g.icon}</span>
                <span style={{ fontSize: 13, fontWeight: 500, color: goals.includes(g.id) ? '#2DB87A' : '#1A1A1A' }}>{g.label}</span>
              </button>)}
            </div>
            {goals.length > 0 && <p style={{ fontSize: 12, color: '#2DB87A', marginTop: 10, fontWeight: 500 }}>{goals.length} area{goals.length > 1 ? 's' : ''} selected</p>}
          </div>}

          {/* STEP 7: Daily Rhythm */}
          {step === 7 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B' }}>Tell me about your daily rhythm.</p></div>
          ...
          {step === 8 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B' }}>Almost done — personality questions.</p></div>
          ...
          {step === 9 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B', lineHeight: 1.5 }}>Do you use any AI tools? I can track your project progress across them.</p></div>
            <div style={{ ...S.card, marginBottom: 12 }}>
              <p style={{ fontSize: 12, color: '#6B6B6B', lineHeight: 1.6, marginBottom: 16 }}>I'll check your recent project chats to give you smart reminders and progress updates. You choose which projects I can see.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {AI_TOOLS.map(t => (
                  <button key={t.id} onClick={() => toggle(aiTools, setAiTools, t.id)} style={{
                    ...
                    background: aiTools.includes(t.id) ? 'rgba(45,184,122,0.08)' : '#FFFFFF',
                    border: `1.5px solid ${aiTools.includes(t.id) ? '#2DB87A' : '#E8E6E3'}`,
                    cursor: 'pointer', transition: 'all 0.2s', textAlign: 'left' as const, width: '100%', fontFamily: "'DM Sans', sans-serif",
                  }}>
                    ...
                    <div style={{ flex: 1 }}><p style={{ fontSize: 14, fontWeight: 600 }}>{t.name}</p><p style={{ fontSize: 11, color: '#6B6B6B' }}>{t.desc}</p></div>
                    {aiTools.includes(t.id) && <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#2DB87A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: 'white', fontSize: 13, fontWeight: 700 }}>✓</span></div>}
                  </button>
                ))}
              </div>
            </div>
          ...
          {step === 10 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B', lineHeight: 1.5 }}>Want world news in your morning briefing?</p></div>
            <div style={{ ...S.card, marginBottom: 14 }}>
              ...
              {newsEnabled && <>
                <div style={{ borderTop: '1px solid #E8E6E3', paddingTop: 16, marginBottom: 16 }}>
                  <p style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>What kind of news?</p>
                  <p style={{ fontSize: 12, color: '#6B6B6B', marginBottom: 12 }}>This shapes how the world feels when you wake up</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div style={{ borderTop: '1px solid #E8E6E3', paddingTop: 16 }}>
                  <p style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>Preferred sources</p>
                  <p style={{ fontSize: 12, color: '#6B6B6B', marginBottom: 12 }}>Optional — or I'll pick the best sources</p>
          ...
          {step === 11 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B', lineHeight: 1.5 }}>How should your morning briefing arrive?</p></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {step === 12 && <div style={{ animation: 'fadeIn 0.5s ease' }}>
            <div style={S.orbMsg}><Orb size={36} animate={false} /><p style={{ fontSize: 16, color: '#6B6B6B' }}>{briefingFormat === 'alarm' || briefingFormat === 'both' ? 'What time should I wake you up?' : 'When should your briefing be ready?'}</p></div>
            <div style={S.card}>
              <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 8 }}>
                {['6:00 AM', '6:30 AM', '7:00 AM', '7:30 AM', '8:00 AM', '8:30 AM'].map(t => <button key={t} onClick={() => setBriefingTime(t)} style={{
                  padding: '10px 18px', borderRadius: 12,
                  background: briefingTime === t ? '#2DB87A' : '#FFFFFF',
                  border: `1.5px solid ${briefingTime === t ? '#2DB87A' : '#E8E6E3'}`,
                  color: briefingTime === t ? 'white' : '#1A1A1A',
                  fontSize: 14, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s', fontFamily: "'DM Sans', sans-serif",
                }}>{t}</button>)}
              </div>
              {briefingTime && <div style={{ marginTop: 16, padding: '12px 16px', borderRadius: 12, background: 'rgba(45,184,122,0.08)', animation: 'fadeIn 0.3s ease' }}>
                <p style={{ fontSize: 13, color: '#2DB87A', fontWeight: 500 }}>
          ...
          {step === 13 && <div style={{ textAlign: 'center', animation: 'fadeIn 0.8s ease' }}>
            ...
            <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 36, fontWeight: 400, marginBottom: 8 }}>You're all set, {name}.</h1>
            <p style={{ fontSize: 16, color: '#6B6B6B', lineHeight: 1.6, maxWidth: 380, margin: '0 auto 28px' }}>I know your rhythm, your people, your goals, and how you like to start your day.</p>
            <div style={{ ...S.card, padding: 24, textAlign: 'left' as const, marginBottom: 28, boxShadow: '0 2px 8px rgba(0,0,0,0.04), 0 12px 32px rgba(0,0,0,0.06)' }}>
              <p style={{ fontSize: 11, fontWeight: 600, color: '#9B9B9B', marginBottom: 14, letterSpacing: 0.4, textTransform: 'uppercase' as const }}>Your profile</p>
              {[
                ...
                <span style={{ fontSize: 12, color: '#9B9B9B' }}>{r.l}</span>
                <span style={{ fontSize: 12, color: '#1A1A1A', fontWeight: 500 }}>{r.v}</span>
              </div>)}
            </div>
            <button onClick={finishOnboarding} disabled={saving} style={{
              padding: '16px 48px', borderRadius: 16,
              background: saving ? '#E8E6E3' : '#2DB87A',
              color: saving ? '#9B9B9B' : 'white', border: 'none',
              fontSize: 16, fontWeight: 600,
              cursor: saving ? 'default' : 'pointer',
              boxShadow: saving ? 'none' : '0 4px 16px rgba(45,184,122,0.3)',
              fontFamily: "'DM Sans', sans-serif",
            }}>
          ...
          {step > 0 && step < 13 && <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 32 }}>
            <button onClick={back} style={{ padding: '10px 20px', borderRadius: 12, background: 'transparent', border: '1px solid #E8E6E3', color: '#6B6B6B', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>← Back</button>
            <span style={{ fontSize: 12, color: '#9B9B9B' }}>{step} of 12</span>
            <button onClick={next} disabled={!canNext()} style={{
              padding: '10px 24px', borderRadius: 12,
              background: canNext() ? '#2DB87A' : '#F5F5F3',
              border: 'none', color: canNext() ? 'white' : '#9B9B9B',
