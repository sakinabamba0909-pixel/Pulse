...
const PRIORITY_COLORS: Record<string, { color: string; dot: string }> = {
  urgent: { color: P.pink,   dot: P.pinkDark   },
  high:   { color: '#D4A47A', dot: '#A07A4A'    },
  normal: { color: P.orchid,  dot: P.orchid     },
  low:    { color: P.green,   dot: P.greenDark  },
};
...
export default function FocusSection({ tasks }: FocusSectionProps) {
  ...
  function toggle(id: string) {
    ...
    // Also call the API to complete the task
    if (!done[id]) {
  ...
  return (
    <div style={{ marginBottom: 48, animation: 'fadeUp 0.6s ease 0.12s both' }}>
      ...
      {tasks.map((t, i) => {
        const d = !!done[t.id];
        const colors = PRIORITY_COLORS[t.priority] ?? PRIORITY_COLORS.normal;
        const barColor = t.projectColor || colors.color;
